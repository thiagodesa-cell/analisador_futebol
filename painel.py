...
# Nova Chave RapidAPI vinculada ao Sofascore
...
# IDs atualizados para o padrão Sofascore
LIGAS_MONITORADAS = {
...
def buscar_estatisticas_time(team_id, league_id, key, data_cache):
    # Como o Sofascore lida com stats agregados de forma diferente, 
    # criamos uma casca protetora de médias padrão caso a API exija sub-endpoints pagos
    ...
    try:
        ...
        # O Sofascore Apidojo retorna médias diretas por jogo na maioria dos planos
        return {
...
def buscar_metricas_completas_avancadas(team_id, key, data_cache):
    # Buscando os últimos jogos via H2H / Team matches para construir a média
    ...
    try:
        ...
        eventos = data.get('events', [])[:8] # Pega os últimos 8 jogos para média
        ...
        for f in eventos:
            f_id = f.get('id')
            time.sleep(0.2) 
            # Pega as stats da partida específica
...
# ==========================================
# PAINEL DE BOTÕES (Lógica de Alertas Mantida)
# ==========================================
...
if st.sidebar.button("💎 Enviar Top Melhores Entradas (Telegram)", key="btn_bilhete_full"):
    ...
    if jogos_hoje:
        ...
        st.sidebar.info(f"⚽ {total_jogos} jogos encontrados. Iniciando o Raio-X avançado via Sofascore...")
...
st.markdown("---")
st.title("⚽ Dashboard Inicializado")
st.success("Motor de dados operando via RapidAPI (Apidojo / Sofascore). Selecione as funções na barra lateral para gerar os relatórios.")
